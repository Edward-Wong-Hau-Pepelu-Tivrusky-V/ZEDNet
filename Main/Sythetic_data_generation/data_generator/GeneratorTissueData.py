...
while counter < args.N: # counter for control-extrusion pairs. N number of samples in total
    ...
    2. Along the centre slice of each layer, measure the number of unique integers to identify candidate extrusions # save candidate extrusion coordinates
    ...
    6. Rotate and Cut out extrusion and control patches # get rid of this. 
    ...
    if len(extrusion_locs) < 5: # go back to step 1 if num of extrusions in tissue less than 5
        logger.debug(f"Generation {generation_counter}: less than 5 Extrusions")
    
    else:
    
        extrusion_locs = np.array(extrusion_locs).astype(int)
        Tissue_extrusion_locs = extrusion_locs
        ...
        extrusion_labels = [
            new_labelled_volumes[z, x, y]
            for z, x, y in extrusion_locs]
    
        extrusion_labels = np.unique(extrusion_labels)
        
        
        # Build OUTSIDE distance (minimum distance to any extrusion cell) 
        extrusion_membrane_distmap = np.full(new_labelled_volumes.shape, np.inf)
        
        for ex_label in extrusion_labels:
        
            outside_dist = ndimage.distance_transform_edt(
                new_labelled_volumes != ex_label,
                sampling=sampling
            )
        
            extrusion_membrane_distmap = np.minimum(
                extrusion_membrane_distmap,
                outside_dist
            )
        
        
        # Build inside distance 
        inside_mask = np.isin(new_labelled_volumes, extrusion_labels)
        
        inside_dist = ndimage.distance_transform_edt(
            ~inside_mask,
            sampling=sampling
        )
        
        # assign negative distances inside the cell
        extrusion_membrane_distmap[inside_mask] = -inside_dist[inside_mask]
        
        
        # Gaussian scaling field params
        scale_peak = np.random.uniform(1.5, 2.0)
        scale_sigma = 1.5
        
        scaling = 1 + (scale_peak - 1) * np.exp(
            -0.5 * (extrusion_membrane_distmap / scale_sigma) ** 2
        )
        ...
        if new_labelled_volumes.max() != labelled_volumes.max() + len(extrusion_locs):
            ...
            synth_img = texture_mask(
                new_labelled_volumes > 0,
                sampler,
                labelled_mask=new_labelled_volumes,
                partitions=None,
                dist_map=None,
                sampling=sampling,
            ...
            # extrusion int scaling 
            synth_img = synth_img * scaling
            ...
            imwrite(f"{args.output_dir}/{str(counter+1).zfill(3)}.tif", synth_img)

            df = pd.DataFrame(Tissue_extrusion_locs, columns=["z", "x", "y"])
            df.to_csv(
                f"{args.output_dir}/{str(counter+1).zfill(3)}_extrusions.csv",
                index=False
                )
